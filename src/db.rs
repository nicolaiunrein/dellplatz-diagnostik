...
impl Db {
    ...
    pub async fn create_user(&self) -> Result<User> {
        let user: Option<UserRecord> = DB.query("CREATE user;").await?.take(0)?;
    ...
    pub async fn evaluate_test(&self, user_id: String) -> Result<Vec<TestResultRecord>> {
        // math::sum(select VALUE (select * from ONLY $this.out.options[$parent.choice].value) from says);
        let res: Vec<TestResultRecord> = DB.query(r#"
            SELECT 
            ...
            ;
        "#).bind(("USER_ID", user_id.clone())).await?.take(0)?;
